...
import { TrendingDown, Calendar, Wallet, CheckCircle2, Zap, Search, TrendingUp, DollarSign } from 'lucide-react';
...
export function Component({
  ...
  return (
    <div className="w-full bg-[#0B1120] text-zinc-100 overflow-hidden rounded-[2.5rem] flex flex-col items-center border border-white/10 py-10 mt-6 shadow-2xl relative select-none">
      ...
      <div className="absolute bottom-0 w-full h-1/3 bg-gradient-to-t from-[#020d1a] to-transparent pointer-events-none" />

      <header className="relative z-10 text-center mb-8 px-4 w-full">
        <div className="inline-flex items-center gap-2 bg-emerald-500/10 text-emerald-400 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 border border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.2)]">
          <Zap size={14} fill="currentColor" /> {t.title}
        </div>
        <h2 className="text-2xl md:text-4xl font-black tracking-tighter text-white uppercase break-words px-2">{t.subtitle}</h2>
      </header>

      {/* CONTROLES */}
      <div className="relative z-20 w-full max-w-5xl px-4 md:px-8 mb-4">
        <div className="bg-white/5 backdrop-blur-xl p-6 rounded-[2rem] border border-white/10 shadow-2xl flex flex-col lg:flex-row gap-6 items-center">
           
           <div className="flex-1 w-full space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest flex items-center gap-2">
                 <Wallet size={16} className="text-emerald-400" /> {t.inputLabel}
              </label>
              <div className="relative group">
                 <span className="absolute left-5 top-1/2 -translate-y-1/2 text-white font-black text-lg">$</span>
                 <input 
                    type="number" 
                    min="0"
                    step="100"
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-2xl pl-10 pr-4 py-3 text-white font-mono text-xl font-bold focus:ring-2 focus:ring-emerald-500 outline-none transition-all group-hover:bg-black/60"
                 />
              </div>
           </div>
           ...
           <div className="flex-1 w-full space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest flex items-center gap-2">
                 <Calendar size={16} className="text-blue-400" /> {t.whenLabel}
              </label>
              <input 
                 type="range" 
                 min="1" 
              ...
              <div className="flex justify-between text-[10px] font-mono text-slate-400">
                  <span>Mês 1</span>
                  <span className="text-blue-400 font-bold">Aporte Mês {paymentMonth}</span>
                  <span>Mês {financingMonths}</span>
              </div>
           </div>

           <div className="bg-emerald-600/20 border border-emerald-500/30 p-4 rounded-2xl min-w-[150px] text-center">
               <span className="text-[9px] font-black text-emerald-200 uppercase tracking-widest block mb-1">{t.resultTitle}</span>
               <div className="flex items-baseline justify-center gap-1">
                   {savedYears > 0 && <span className="text-2xl font-black text-white tracking-tighter">{savedYears}<span className="text-xs ml-1">{t.years}</span></span>}
                   {savedRemainingMonths > 0 && <span className="text-xl font-black text-emerald-100 tracking-tighter ml-1">+{savedRemainingMonths}<span className="text-xs ml-1">{t.months}</span></span>}
                   {simulation.monthsSaved === 0 && <span className="text-xl font-bold text-white/50">-</span>}
               </div>
           </div>
        </div>
      </div>
      ...
      <div className="flex gap-6 mt-4 mb-2">
         <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-red-500/50 border border-red-500 border-dashed"></div>
            <span className="text-[10px] uppercase font-bold text-red-400 tracking-widest">{t.scenarioStandard}</span>
         </div>
         <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-emerald-500 border border-emerald-400 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></div>
            <span className="text-[10px] uppercase font-bold text-emerald-400 tracking-widest">{t.scenarioSmart}</span>
         </div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-6xl px-4 h-[400px] mt-2 group cursor-crosshair">
        ...
        >
          ...
          {[0.2, 0.4, 0.6, 0.8].map(ratio => (
             <line key={ratio} x1="0" y1={chartHeight * ratio} x2={chartWidth} y2={chartHeight * ratio} stroke="white" strokeOpacity="0.05" />
          ))}

          {/* Marcadores Verticais (Linhas Segmentadas) - Opacidade Aumentada */}
          {simulation.data.slice(0, visibleMonths).map((p, i) => {
             ...
             if (i % 12 === 0 || i === paymentMonth || i === simulation.endMonthSmart) {
                 ...
                 return (
                    <g key={i}>
                        <line x1={x} y1="0" x2={x} y2={chartHeight} stroke="white" strokeOpacity={0.15} strokeDasharray="3,3" />
                        {i % 12 === 0 && (
          ...
          {selectedPoint && (
              <g transform={`translate(${getX(selectedPoint.month)}, 0)`}>
                  <line x1="0" y1="0" x2="0" y2={chartHeight} stroke="#38bdf8" strokeWidth="1" />
                  <circle cy={getY(selectedPoint.balanceSmart)} r="6" fill="#38bdf8" stroke="white" strokeWidth="2" />
                  <circle cy={getY(selectedPoint.balanceStandard)} r="4" fill="#ef4444" stroke="white" strokeWidth="1" opacity="0.5" />
              </g>
          )}
        ...
        <AnimatePresence>
            {selectedPoint && (
                <MotionDiv 
                    ...
                    className="absolute top-4 left-1/2 -translate-x-1/2 md:left-auto md:right-8 md:translate-x-0 w-[300px] bg-slate-900/95 backdrop-blur-xl border border-white/20 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.7)] p-5 z-50 pointer-events-none"
                >
                    <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-2">
                        <div className="flex items-center gap-2">
                            <Search size={14} className="text-aqua-400" />
                            <span className="text-xs font-black text-white uppercase tracking-widest">{t.tableTitle} {selectedPoint.month}</span>
                        </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center">
                            <span className="text-[10px] font-bold text-red-400 uppercase">{t.marketCost}</span>
                            <span className="text-sm font-mono font-bold text-red-400">${Math.round(selectedPoint.realityMonthly)}</span>
                        </div>

                        {/* Linha 2: Custo Aquafeel */}
                        <div className="flex justify-between items-center relative">
                            <span className="text-[10px] font-bold text-blue-400 uppercase">{t.programCost}</span>
                            <span className="text-sm font-mono font-bold text-blue-400">${Math.round(selectedPoint.programMonthly)}</span>
                            <div className="absolute top-1/2 left-0 w-full h-px bg-blue-400/10 -z-10"></div>
                        </div>

                        {/* Linha Extra: Pagamento Extra */}
                        {selectedPoint.isExtraMonth && (
                            <div className="bg-emerald-500/20 border border-emerald-500/30 rounded-lg p-2 flex justify-between items-center animate-pulse">
                                <span className="text-[9px] font-black text-emerald-400 uppercase flex items-center gap-1">
                                    <Zap size={10} /> {t.extraPaid}
                                </span>
                                <span className="text-xs font-mono font-black text-white">+${extraPayment}</span>
                            </div>
                        )}
                        ...
                        <div className="flex justify-between items-end">
                            <span className="text-[10px] font-black text-slate-400 uppercase">{t.balance}</span>
                            <div className="text-right">
                                <span className={`text-xl font-black font-mono tracking-tighter block ${selectedPoint.balanceSmart === 0 ? 'text-emerald-400' : 'text-white'}`}>
                                ...
                                {selectedPoint.balanceSmart < selectedPoint.balanceStandard && selectedPoint.balanceSmart > 0 && (
                                    <span className="text-[9px] text-red-400 line-through opacity-60 block">
                        ...
                        {selectedPoint.balanceSmart === 0 && (
                            <div className="mt-2 text-center bg-emerald-600 text-white text-[10px] font-black uppercase py-1 rounded-md animate-bounce shadow-lg">
                                DÍVIDA QUITADA!
                            </div>
                        )}
                    </div>
