  ...
  return (
    <div className="max-w-7xl mx-auto px-2 md:px-4 relative z-20">
      <div className="grid md:grid-cols-2 gap-0 shadow-2xl rounded-3xl overflow-hidden ring-4 ring-slate-100">
        ...
        <div className={`text-white p-6 md:p-10 flex flex-col relative overflow-hidden transition-colors duration-500 ${isExpired ? 'bg-slate-900' : 'bg-[#020d1a]'}`}>
          ...
          <div className="relative z-10 h-full flex flex-col">
            <div className="text-center mb-6">
              ...
              <p className={`text-xs font-bold tracking-widest uppercase mt-1 ${isExpired ? 'text-red-400' : 'text-amber-400'}`}>{t.solutionSub}</p>
            </div>

            <div className="text-center mb-8 relative">
                <div className="flex flex-col items-center">
                    <div className={`text-7xl font-black text-white tracking-tighter flex items-start gap-1 ${isExpired ? 'text-slate-600' : 'text-white'}`}>
                        <span className="text-3xl mt-2">$</span>
                        {currentPlan.amount}
                    </div>
                    {/* Cashback só aparece em destaque para o plano CASH */}
                    {!isExpired && selectedPlan === 'cash' && (
                        <div className="mt-4 bg-red-600 text-white px-6 py-2 rounded-full font-black text-lg animate-bounce shadow-[0_0_30px_rgba(220,38,38,0.5)] border-2 border-white/20 flex items-center gap-2">
                           <Gift size={20} /> $1.000 CASHBACK
                        </div>
                    )}
                </div>
            </div>
                  ...
                  className={`flex flex-col items-center justify-center p-2 rounded-xl border transition-all duration-200 w-[48%] sm:w-[30%] md:w-auto md:flex-1 ${
                    selectedPlan === plan.id 
                      ? (isExpired 
                          ...
                          : 'bg-white text-aqua-950 border-white shadow-lg scale-105 relative z-10')
            ...
            {/* Benefits List */}
            <div className={`rounded-2xl p-6 mb-8 border ${isExpired ? 'bg-red-900/10 border-red-900/30' : 'bg-white/5 border-white/10'}`}>
                <div className="grid grid-cols-1 gap-4 text-sm">
                    ...
                    {isExpired ? (
                        ...
                        <div className="flex flex-col border-b border-white/5 pb-2">
                            <div className="flex justify-between items-center">
                              <span className="flex items-center gap-2 text-red-400 font-black"><Sparkles size={16}/> {t.benefits.discount}</span>
                    ...
                    <div className="flex flex-col pt-1 bg-white/5 p-3 rounded-xl border border-white/5">
            ...
            <div className="mt-auto space-y-4">
              <a 
                ...
                className={`w-full font-black text-xl py-5 rounded-2xl shadow-2xl transform transition-all hover:-translate-y-1 flex items-center justify-center gap-3 cursor-pointer group ${
                    isExpired 
                    ? 'bg-red-600 hover:bg-red-50 text-white' 
              ...
              <p className={`text-center text-xs font-bold uppercase tracking-widest ${isExpired ? 'text-red-400' : 'text-amber-400'}`}>
