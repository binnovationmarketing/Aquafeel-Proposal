    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-20">
      <div className="flex flex-col lg:flex-row shadow-2xl rounded-[2.5rem] overflow-hidden border border-slate-100 bg-white">
        ...
        <div className="w-full lg:w-1/2 p-6 md:p-10 border-b lg:border-b-0 lg:border-r border-gray-100 flex flex-col justify-between bg-slate-50/50">
          <div>
            <div className="mb-6 text-center md:text-left">
              ...
              <p className="text-[9px] font-black text-slate-400 tracking-[0.3em] uppercase mt-2">{t.problemSub}</p>
            </div>
          ...
          <div className="bg-slate-900 rounded-[2rem] p-8 mt-8 text-center relative shadow-xl overflow-hidden group">
            ...
            <div className="text-4xl md:text-5xl font-black text-white tracking-tighter">${monthlyTotalSpending.toFixed(2)}</div>
            <div className="mt-4 inline-flex items-center gap-2 bg-red-600 text-white px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest animate-pulse">
              <AlertTriangle size={12} /> {t.waste}
            </div>
          </div>
        </div>
        ...
        <div className={`w-full lg:w-1/2 p-6 md:p-10 flex flex-col relative ${isExpired ? 'bg-slate-900' : 'bg-slate-950 text-white'}`}>
          <div className="mb-6">
            <h2 className="text-xl font-black uppercase tracking-tighter leading-none">{t.solutionTitle}</h2>
            <p className="text-[9px] font-black tracking-[0.3em] uppercase mt-2 text-aqua-400">{t.solutionSub}</p>
          </div>
          ...
          <div className="flex flex-col items-center mb-6 py-4">
              <div className="text-6xl md:text-7xl font-black text-white tracking-tighter flex items-start gap-1">
                  ...
                  {currentPlan.id !== 'cash' && <span className="text-lg text-slate-600 mt-auto mb-4 font-bold">/mo</span>}
              </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {plans.map((p) => (
              <button
                ...
                className={`py-3.5 rounded-xl border text-[10px] font-black uppercase transition-all ${
                  selectedPlan === p.id ? 'bg-white text-slate-950 border-white shadow-lg transform scale-105' : 'bg-white/5 border-white/10 text-slate-500 hover:bg-white/10'
                }`}
              >
                {p.label}
              </button>
          ...
          <div className="mb-8 overflow-hidden">
            <label className="block text-[8px] font-black text-slate-500 uppercase tracking-widest mb-3">SCORE DE CRÉDITO</label>
            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar scroll-smooth">
              {creditOptions.map((opt) => (
                <button
                  key={opt.id} onClick={() => setSelectedCredit(opt.id as CreditRangeId)}
                  className={`flex flex-col min-w-[100px] p-3 rounded-xl border transition-all shrink-0 ${
                    selectedCredit === opt.id ? 'bg-aqua-500 border-aqua-400 text-white shadow-lg' : 'bg-white/5 border-white/10 text-slate-500 hover:bg-white/10'
                  }`}
                >
                  <span className="text-[9px] font-black opacity-70 mb-1">{opt.score}</span>
          ...
          <button onClick={onOpenAnalyst} className="w-full bg-red-600 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-3 hover:bg-red-500 transition-all active:scale-[0.98] shadow-xl mt-auto">
